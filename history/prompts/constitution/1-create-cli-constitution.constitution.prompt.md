...
prompt_text: |
  ...
  Create a Constitution for this project using Spec-Driven Development.

  Project Type:
  - Python console (CLI) application

  Core Rules:
  - Development must strictly follow: Spec → Plan → Tasks → Implement
  - No code may be written outside an approved spec
  - Manual edits are not allowed
  - Each phase must be completed and validated before moving forward

  Application Constraints:
  - Python only
  - Console / terminal based interaction
  - No web frameworks
  - No frontend or UI libraries
  - No database, files, or external storage
  - In-memory data structures only

  Scope Control:
  - No features beyond the approved spec
  - No additional files or folders unless explicitly specified
  - Keep structure minimal and readable

  Completion Criteria:
  - Application runs from the terminal without errors
  - All specified features work via CLI
  - Clear prompts and outputs for the user
  - Clean exit without crashes

  Enforcement:
  - Reject any implementation that violates scope or constraints
  - Flag misplaced files or unnecessary abstractions
response_text: |
  Created a comprehensive constitution for the Python CLI application project that enforces Spec-Driven Development principles and the specified constraints. The constitution includes six core principles: Spec-Driven Development, Python-Only Implementation, Console/Terminal Interface, In-Memory Data Management, Scope Adherence, and Clean Implementation. It also defines development constraints, workflow requirements, and governance rules.
outcome: 
evaluation: 
---
